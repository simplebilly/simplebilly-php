# OpenAPI\Client\GenerateXrechnungApi
...
```php
generateXrechnungApi($id, $supplier_name, $supplier_street, $supplier_city, $supplier_zip, $supplier_country, $supplier_vat_id): \OpenAPI\Client\Model\XRechnungResponse
```
...
$apiInstance = new OpenAPI\Client\Api\GenerateXrechnungApi(
...
try {
    $result = $apiInstance->generateXrechnungApi($id, $supplier_name, $supplier_street, $supplier_city, $supplier_zip, $supplier_country, $supplier_vat_id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling GenerateXrechnungApi->generateXrechnungApi: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\OpenAPI\Client\Model\XRechnungResponse**](../Model/XRechnungResponse.md)
