<?php
/**
 * MovementType
 *
 * PHP version 8.1
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
 */

/**
 * SimpleBilly API
 *
 * Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
 *
 * The version of the OpenAPI document: 0.1.0
 * Contact: support@simplebilly.com
 * Generated by: https://openapi-generator.tech
 * Generator version: 7.25.0
 */

/**
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

namespace OpenAPI\Client\Model;
use \OpenAPI\Client\ObjectSerializer;

/**
 * MovementType Class Doc Comment
 *
 * @category Class
 * @description Stock movement type.
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
 */
class MovementType
{
    /**
     * Possible values of this enum
     */
    public const GOODS_RECEIPT = 'goods_receipt';

    public const GOODS_RECEIPT_REVERSAL = 'goods_receipt_reversal';

    public const TRANSFER_IN = 'transfer_in';

    public const TRANSFER_OUT = 'transfer_out';

    public const RETURN_RESTOCK = 'return_restock';

    public const PRODUCTION_CONSUMPTION = 'production_consumption';

    public const PRODUCTION_COMPLETION = 'production_completion';

    public const INVENTORY_ADJUSTMENT = 'inventory_adjustment';

    public const SALE = 'sale';

    public const OTHER = 'other';

    /**
     * Gets allowable values of the enum
     * @return string[]
     */
    public static function getAllowableEnumValues()
    {
        return [
            self::GOODS_RECEIPT,
            self::GOODS_RECEIPT_REVERSAL,
            self::TRANSFER_IN,
            self::TRANSFER_OUT,
            self::RETURN_RESTOCK,
            self::PRODUCTION_CONSUMPTION,
            self::PRODUCTION_COMPLETION,
            self::INVENTORY_ADJUSTMENT,
            self::SALE,
            self::OTHER
        ];
    }
}


